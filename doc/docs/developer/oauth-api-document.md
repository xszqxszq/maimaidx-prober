...
| `https://auth.diving-fish.com/device` | 设备码绑定页：用户在此输入用户码并确认授权。若该次绑定提交了 `handoff=code` ，用户在确认后还会于此页得到一串确认码 |
...
**公开客户端的响应中还包含 `refresh_token`** ，此后凭它按 [4.2 节](#42-refresh_token-刷新令牌)自行续期。这是公开客户端取得长期访问能力的唯一途径：它不能使用换票，因此**必须以轮询或[确认码](#45-confirmation-code-确认码换取令牌)取回令牌**，且必须为每位用户各自保存这把令牌。机密客户端不在此列——它的 `grant_type` 中不含 `refresh_token` ，续期请走换票。

### 4.5 confirmation-code 确认码换取令牌

设备码绑定的另一种收尾方式：由用户将授权完成页上显示的**确认码**发回给您的应用，您凭该码换取一次令牌，无需轮询。

使用本方式须在发起绑定时提交 `handoff=code` ，见 [5.1 节](#51-请求参数)。

| **参数** | **必填** | **说明** |
|-----|-----|-----|
| `grant_type` | 是 | `urn:diving-fish:params:oauth:grant-type:confirmation-code` |
| `confirmation_code` | 是 | 用户发回给您的确认码，形如 `BCDF-GHJK-LMNP` |
| `client_id` | 是 | 应用标识 |
| `client_secret` | 视情况 | 机密客户端必填 |

确认码不区分大小写，连字符与首尾空白均可省略。用户从网页复制并转发的过程中常会引入这些差异，您无需自行处理。

服务端在签发前会依次校验三项，缺一不可：

1. 该确认码存在且未被使用；
2. 该码出自本应用发起的绑定；
3. 用户对本应用的授权仍然有效，且涵盖本次绑定申请的范围。

响应与 [4.4 节](#44-device_code-设备码换取令牌)的字段完全一致，同样包含 `sub` ，公开客户端同样包含 `refresh_token` 。

带有 `handoff=code` 的绑定，其 `device_code` **不可再用于轮询**， [4.4 节](#44-device_code-设备码换取令牌)的请求将返回 `invalid_request` 。两种收尾方式只能择一，原因见下方提示。

用户点击拒绝是唯一的例外：此时轮询仍返回 `access_denied` ，以便您及时结束等待。

:::warning
确认码是一次性凭据，且只能由发起该次绑定的应用兑换。兑换成功、超过有效期（10 分钟）或用户重新发起绑定后，原码即失效。
:::

:::tip
**公开客户端建议一律使用本方式。**

设备码端点不校验凭据，任何人都可以用您的 `client_id` 生成一条绑定链接并转发给他人。在轮询方式下，令牌交付给发起该次请求的一方，也就是链接的制作者；公开客户端在此交付的还是长期有效的 `refresh_token` 。

改用确认码后，该码只出现在完成授权的那位用户的浏览器中。未经其转交，绑定无法完成，冒名者也就无从取得令牌。

两种方式若同时可用，冒名者可在提交 `handoff=code` 之后照常轮询，上述保护即不成立。这是二者必须互斥的原因。
:::
...
| **error** | **状态码** | **含义** |
|-----|-----|-----|
| `invalid_grant` | 400 | 确认码不存在、已被使用、已过期、出自其他应用，或用户已撤销授权。以上情形不作区分 |
| `invalid_client` | 401 | `client_id` 或 `client_secret` 有误，或应用已停用 |
| `invalid_request` | 400 | `confirmation_code` 缺失 |
| `unauthorized_client` | 400 | 该应用未被允许使用设备码绑定 |
| `slow_down` | 429 | 兑换过于频繁，见 [第 10 节](#10-配额与限流) |

:::info
确认码不存在、已被使用与出自其他应用三种情形返回同一个错误，是有意为之。若可以区分，逐一尝试的一方便可据此确认某串码是否有效。

对您而言三种情况的处理方式相同：提示用户重新发起一次绑定。
:::
...
| `handoff` | 否 | 绑定的收尾方式，取值为 `poll` （默认，由您轮询令牌端点）或 `code` （由用户将确认码发回给您，见 [4.5 节](#45-confirmation-code-确认码换取令牌)）。取值仅此两种，其余一律返回 `invalid_request` |
...
**更彻底的做法是提交 `handoff=code`** ，改由用户回填确认码完成绑定。此时即便冒名者生成了链接、用户也点击了同意，令牌仍不会交付给他：确认码只出现在该用户的浏览器中。见 [4.5 节](#45-confirmation-code-确认码换取令牌)。
:::
...
请求中提交了 `handoff=code` 时，响应中额外包含 `"handoff": "code"` ，其余字段不变。此时用户点击同意后不会被提示返回应用，而是得到一串确认码，需由他发回给您，您据此按 [4.5 节](#45-confirmation-code-确认码换取令牌)换取令牌。

对机密客户端而言，用户点击同意的那一刻，授权记录与 `subject_ref` 的映射即告建立，因此**轮询并非必需**：您可以不理会 `device_code` ，下一次换票自然会成功。轮询的价值在于明确知晓用户点击的是同意还是拒绝。

公开客户端则**必须取回令牌**：它无法换票，用户令牌只在这一次响应中出现。取回方式有二，轮询或提交 `handoff=code` 改由用户回填确认码，推荐后者，理由见 [4.5 节](#45-confirmation-code-确认码换取令牌)。
...
| 轮询设备码返回 400 `invalid_request` ，提示 `handoff=code` | 该次绑定以确认码收尾，不能轮询，见 [4.5 节](#45-confirmation-code-确认码换取令牌) |
| 兑换确认码返回 400 `invalid_grant` | 确认码已过期（超过 10 分钟）、已被使用，或与 `client_id` 不匹配 |
